...
public abstract class MicroService implements Runnable {
    ...
    private ConcurrentHashMap< Class<? extends Message>,Callback > messageCallback;    //to know which callback to use for every event type
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        ...
        msgBus.subscribeEvent(type,this);   //let msgBus know im handling the event
        if(!messageCallback.containsKey(type)){messageCallback.put(type,callback);}
    }
    ...
    protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
        msgBus.subscribeBroadcast(type,this);   //let msgBus know im handling the brodcast
        if(!messageCallback.containsKey(type)){messageCallback.put(type,callback);}
    }
    ...
    public final void run() {
        ...
        initialize();
        while (!terminated) {
            Message msg_To_handle;
            try {
                msg_To_handle=msgBus.awaitMessage(this);
                messageCallback.get(msg_To_handle.getClass()).call(msg_To_handle);
